...
\begin{frame}{Verwandte Arbeiten: Segmentierung nach Kanezaki\fcite{kanezaki2018_unsupervised_segmentation} (1)}
\begin{itemize}
...
\begin{frame}{Verwandte Arbeiten: Segmentierung nach Kanezaki\fcite{kanezaki2018_unsupervised_segmentation} (2)}
\begin{figure}
	\includegraphics[width=\textwidth,keepaspectratio]{kanezaki.png}
	\caption{Vorgehensweise nach Kanezaki\fcite{kanezaki2018_unsupervised_segmentation}}
\end{figure}
\end{frame}

\begin{frame}{Verwandte Arbeiten: Segmentierung nach Kanezaki\fcite{kanezaki2018_unsupervised_segmentation} (3)}
\begin{figure}
	\includegraphics[width=\textwidth,keepaspectratio]{kanezaki.png}
...
\begin{frame}{\textit{Detection of craters [...] using shape and texture features} nach Bandeira\fcite{Bandeira2012}}
\begin{columns}
	\begin{column}{.5\textwidth}
		\begin{enumerate}
		\item Vorsortierung basierend auf Urbach \& Stepinskis Algorithmus\fcite{urbach2009automatic} zur Kratererkennung
		\begin{itemize}
	...
	\begin{column}{.5\textwidth}
		\begin{figure}[H]
			...
			\caption{Erkannte Krater nach Bandeira\fcite{Bandeira2012}}
		\end{figure}
	\end{column}
\end{columns}
\bigskip
\begin{center}Hierauf basierend: Der Krater-Datensatz der University of Massachusetts\fcite{umass_craters} (vergleichbar mit unserem Ziel)
\end{center}
\end{frame}

\begin{frame}{Verwandte Arbeiten: \textit{Crater Detection via CNNs}\fcite{2016arXiv160100978C}}
\begin{itemize}
	\item Erforscht Neuronale Netze statt manuell erstellten Filtern
	\item \enquote{Klassischer} Ansatz über neuronale Netze
	\item Lernt auf Bandeira-Datensatz der UMass\fcite{umass_craters}
	...
	\begin{columns}
		...
		\begin{column}{.3\textwidth}
			\begin{figure}[H]
				...
				\caption{Beispiel für einen \enquote{CTX-Streifen}\fcite{doi:10.1029/2006JE002808}}
			\end{figure}
		\end{column}
	\end{columns}	
\end{frame}
...
\begin{columns}
	...
	\begin{column}{.5\textwidth}
		\begin{figure}[H]
			...
			\caption{Anomalien der Marsoberfläche, aus \fcite{sheriff_2019}}
		\end{figure}
	\end{column}
\end{columns}	
\end{frame}
...
\begin{itemize}
	\item Grundlegende Idee ähnlich zu Kanezaki\fcite{kanezaki2018_unsupervised_segmentation}, basierend auf PyTorch
	...
	\item Für bessere Performance wird oft auf Cython\fcite{behnel2010cython} zurück gegriffen
\end{itemize}	
\end{frame}
...
\medskip
\begin{itemize}
	\item \textit{Common Objects In Context}\fcite{LMBHPRDZ:ECCV:2014}
	\item \textit{Cityscapes Dataset}\fcite{Cordts2016Cityscapes}
	\item[$\Rightarrow$] Weit verbreitete Datensätze zur Bildsegmentierung
	\medskip
	\item \textit{The Prague Texture Segmentation Datagenerator and Benchmark}\fcite{mikevs2015benchmarking}
	\item[$\Rightarrow$] Zufällig generierte Datensätze zur Clusteringanalyse
	\medskip
	\item \textit{Crater Dataset} der UMass Boston
	 KDLab\fcite{umass_craters}
